...
05_predict.py
-------------
Predict P(home team wins) for a single matchup using either of the two
trained models.
...
Use NBA 3-letter abbreviations: BOS, LAL, DEN, GSW, MIA, MIL, PHI, NYK, ...
"""
...
def latest_stats(df: pd.DataFrame, team: str) -> dict:
    """Pull the most recent rolling stats for a team, regardless of side."""
...
def predict_with_model(model_name: str, X) -> float:
    """Return P(home_wins) using the selected model. X is a DataFrame for
    sklearn/xgboost, or a numpy array for the neural net."""
...
def main() -> None:
    ...
    ap.add_argument("--home", required=True, help="Home team abbrev (e.g. BOS)")
    ap.add_argument("--away", required=True, help="Away team abbrev (e.g. LAL)")
    ...
    print("\nCompare these fair lines to DraftKings / FanDuel / Kalshi.")
    print("If a sportsbook offers worse odds than fair on your pick, that's +EV.\n")
